...
#win percentage locations
H_odds = (HomeWin/100) * 20
draw_odds = (draw/100) * 20 + H_odds

away_win = 41.15 + draw_odds + (20 - draw_odds)/2 
draw_percent = 41.15 + H_odds + (draw_odds - H_odds)/2 
home_win = 41.15 + H_odds/2

plt.text(home_win,52.75,'{:2.0f}%'.format(HomeWin),**font,color="black",zorder=zo,fontsize=9)
plt.text(draw_percent,52.75,'{:2.0f}%'.format(draw),**font,color="black",zorder=zo,fontsize=9)
plt.text(away_win,52.75,'{:2.0f}%'.format(AwayWin),**font,color="black",zorder=zo,fontsize=9)
